...
\usetikzlibrary{calc,arrows.meta,positioning,patterns,angles,quotes,babel,decorations.pathmorphing,shapes.geometric,shapes.misc}
\usepackage{fancyhdr}
...
\begin{figure}[ht]
	...
	\caption{Effect van thermisch proces op materialen}
	\label{fig:image9}
\end{figure}
...
tijdens het bewerken van een materiaal.
\begin{itemize}
...
\begin{itemize}
	...
	\item Wighoek $\beta$ $\to$ zo groot mogelijk maken.
	      \subitem Wighoeken bepalen de sterkte van de beitel en de warmteafvoer. Grote wighoeken brengen de warmte sneller weg
	      ...
	      \begin{figure}[ht]
		      ...
		      \label{fig:image12}
	      \end{figure}
	\item Vrijloophoek $\alpha$ $\to$ groter $\to$ minder wrijving tussen werkstuk en beitel. Tussen 6° en 10°
	      \subitem Vrijloophoek moet er zijn zodat je beitel niet begint te wrijven over het oppervlakte van je materiaal.
...
\subsection{Krachten}
Tijdens het bewerken van materialen met een beitel
komen er verschillende krachten op het werkstuk en de beitel te staan.
...
\begin{itemize}
	...
	\item Terugdrukkracht (Fp): Kracht die nodig is om de beitel in het werkstuk te duwen.
	      \begin{figure}[ht]
		      \centering
...
\begin{itemize}
	\item \textbf{Opbouwlaag (build-up edge/BUE)}
	      ...
	      opbouwt aan de punt van de beitel. Zie video toledo.
	...
	\item \textbf{Warmte}
	      ...
	      Warmte wordt op het werkstuk op drie plaatsen gecreëerd:
	      1. Afschuifvlak (shear zone):
	      2. Secundaire afschuifvlak (tussen spaan en beitel):
	      3. Vrijloopvlak (tussen werkstuk en beitel):
...
Met welk materiaal ga je je beitel maken?
...
\end{figure}
Al deze snijmaterialen zijn gecreëerd door de jaren heen.
...
\frm{Kienzle vergelijking Hoofdsnijkracht}{F_c = k_c \cdot a\cdot f^{(1-e)}}{waarbij $k_c$ (\si{N/mm^2} of \si{Pa}) de snijkrachtcoëfficiënt is, $a$ de snedediepte, $f$ de voeding en $e$ de snijkrachtexponent.}
...
De oppervakteruwheid wordt gecreerd door de beitel. De beitel is niet perfect scherp en is dus licht bol, dit is de neusradius *$r_\epsilon$.
...
\frm{Voedingskracht}{F_f = k_f \cdot a \cdot f^{(1-e)}}{waarbij $k_f$ de voedingskrachtcoëfficiënt is, $a$ de snedediepte, $f$ de voeding en $e$ de voedingskrachtsexponent.}
\frm{Verspanningsmoment}{M_c = C_m \cdot d^{x_M} \cdot f^{y_M}}{waarbij $C_m$ de momentcoëfficiënt is, $d$ de diameter, $f$ de voeding en $x_M$, $y_M$ de momentexponenten.}
...
\frm{Snijkracht bij Frezen}{F_c = k_c \cdot b \cdot h_{gem}^{(1-e)} \cdot z_i}{waarbij $k_c$ de snijkrachtcoëfficiënt is, $b$ de snedebreedte, $h_{gem}$ de gemiddelde snededikte, $z_i$ het aantal tanden en $e$ de snijkrachtsexponent.}

\frm{Snijmoment bij Frezen}{M_c = C_m \cdot d^{x_M} \cdot f_z^{y_M} \cdot z_i}{waarbij $C_m$ de momentcoëfficiënt is, $d$ de diameter, $f_z$ de voeding per tand, $z_i$ het aantal tanden en $x_M$, $y_M$ de momentexponenten.}

\frm{Voedingskracht bij Frezen}{F_f = k_f \cdot b \cdot h_{gem}^{(1-e)} \cdot z_i}{waarbij $k_f$ de voedingskrachtcoëfficiënt is, $b$ de snedebreedte, $h_{gem}$ de gemiddelde snededikte, $z_i$ het aantal tanden en $e$ de voedingskrachtsexponent.}
...
\frm{Pulsenergie tijdens vonkfrezen}{W_e = \int_0^{t_e} U_e I_e \, dt}{waarbij $U_e$ de doorslagspanning is, $I_e$ de stroom en $t_e$ de pulstijd.}
\begin{figure}[ht]
	\centering
...
\frm{Gereedschapslijtage bij EDM}{v = \frac{\text{Volume gereedschap versleten}}{\text{Volume werkstuk verspaand}}}{Deze is typisch tussen $1\%$ en $5\%$.}
...
\frm{Erosievastheid van elektrode (schematische benadering)}{E = \lambda \cdot \rho \cdot c \cdot T_m}{waarbij \(\lambda\) de warmtegeleidingscoëfficiënt is, \(\rho\) de massadichtheid, \(c\) de soortelijke warmte en \(T_m\) de smelttemperatuur.}
...
\frm{Afnamesnelheid bij ECM}{M_{\mathrm{ECM}} = \dfrac{I \cdot A}{F \cdot V}\cdot \eta}{waarbij MMR(Material Removal Rate) is de snelheid van afname, \(I\) de stroomsterkte (A), \(A\) de atoommassa van het materiaal (g/mol), \(V\) de valentie (aantal elektronen), \(F\) de Faraday-constante (\(96485\,\mathrm{C/mol}\)) en \(\eta\) de efficiëntie.}
...
\frm{BBP = Beam Parameter Product}{BBP = \omega_0 \cdot \dfrac{\theta}{2}}{waarbij \(\omega_0\) de halve diameter van het focuspunt is en \(\theta\) de divergerende hoek van de straal.}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.8\textwidth]{Laserfocussen.png}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.6\textwidth]{NC-lasers.png}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.6\textwidth]{Kleinefiguurdoorlasersnijden.png}
...
\newline
\frm{Afschuifkracht bij ponsen}{\ensuremath{F = \tau \cdot L \cdot s_0}}{Waarbij \(\!F\!\) de afschuifkracht is, \(\tau\) de schuifspanning van het materiaal,
	\(L\) de omtrek van het profiel en \(s_0\) de plaatdikte.}
...
\frm{Schuifkracht bij knippen}{\ensuremath{F = 0.5 \cdot \dfrac{s^2}{\tan\varepsilon} \cdot \tau}}{waarbij \(F\) de afschuifkracht is, \(s\) de plaatdikte, \(\varepsilon\) de snijhoek en \(\tau\) de schuifspanning van het materiaal.}
...
{waarbij \(F\) de afschuifkracht is, \(s_0\) de plaatdikte, \(l\) de omtrek van het profiel en \(\tau\) de schuifspanning van het materiaal.}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.6\textwidth]{Schuifspanningen bij Ponsen.png}
...
\frm{Tweede formule Schuifkracht bij ponsen}{\ensuremath{F = 1,\cdot y_s \cdot s_0 \cdot l \cdot \tau \cdot k_s}}
{waarbij \(F\) de afschuifkracht is, \(y_s\) een correctiefactor voor het ponsproces, \(\tau\) de schuifspanning van het materiaal,
	\(s_0\) de plaatdikte, \(l\) de omtrek van het profiel en \(k_s\) de specifieke snijkracht.}
...
Werkvoorbereiding is de brug tussen ontwerp en productie. Het zorgt ervoor dat het juiste product, op het juiste moment, met de juiste kwaliteit en tegen de laagste kosten wordt gemaakt.
...
\begin{figure}[ht]
	...
	\begin{tikzpicture}[
			...
			process/.style={rectangle, minimum width=3cm, minimum height=1cm, text centered, draw=black, fill=orange!10, thick},
			decision/.style={diamond, aspect=2, minimum width=3cm, minimum height=1cm, text centered, draw=black, fill=green!10, thick},
			arrow/.style={thick,->,>=stealth}
		]

		\node (cad) [startstop] {Productontwerp (CAD)};
		\node (check) [decision, below of=cad, yshift=-0.5cm] {Maakbaar?};
		\node (plan) [process, below of=check, yshift=-0.5cm] {Procesplanning (Routing)};
		\node (res) [process, below of=plan] {Middelen (Tools/CNC)};
		\node (sched) [process, below of=res] {Productieplanning};
		\node (make) [startstop, below of=sched] {Start Productie};

		\draw [arrow] (cad) -- (check);
		\draw [arrow] (check) -- node[anchor=east] {Ja} (plan);
		\draw [arrow] (check.east) -- ++(1.5,0) |- node[anchor=south, xshift=-0.5cm] {Nee (Redesign)} (cad.east);
		\draw [arrow] (plan) -- (res);
		\draw [arrow] (res) -- (sched);
		\draw [arrow] (sched) -- (make);

	\end{tikzpicture}
	\caption{Flowchart van het werkvoorbereidingsproces.}\label{fig:werkvoorbereiding_flow}
\end{figure}


\chapter{Productiegericht ontwerpen}

Productiegericht ontwerpen (Design for Manufacturing) betekent dat je al in de ontwerpfase rekening houdt met de beperkingen en mogelijkheden van het productieproces.
...
\begin{figure}[ht]
	...
	\begin{tikzpicture}[
			node distance=2cm,
			every node/.style={font=\small},
			block/.style={rectangle, rounded corners, minimum width=2.5cm, minimum height=1cm, text centered, draw=black, fill=blue!10, thick},
			decision/.style={diamond, aspect=2, minimum width=2.5cm, minimum height=1cm, text centered, draw=black, fill=yellow!10, thick},
			line/.style={draw, thick, ->, >=stealth}
		]

		\node (eisen) [block] {Eisen \& Functies};
		\node (ontwerp) [block, below of=eisen] {Concept Ontwerp};
		\node (dfm) [decision, below of=ontwerp, yshift=-0.5cm] {Maakbaar?};
		\node (proto) [block, below of=dfm, yshift=-0.5cm] {Prototype};
		\node (test) [decision, below of=proto, yshift=-0.5cm] {Test OK?};
		\node (final) [block, below of=test, yshift=-0.5cm] {Definitief Ontwerp};

		\draw [line] (eisen) -- (ontwerp);
		\draw [line] (ontwerp) -- (dfm);
		\draw [line] (dfm) -- node[anchor=east] {Ja} (proto);
		\draw [line] (dfm.east) -- ++(2,0) |- node[anchor=south, xshift=-0.5cm] {Nee (Aanpassen)} (ontwerp.east);
		\draw [line] (proto) -- (test);
		\draw [line] (test) -- node[anchor=east] {Ja} (final);
		\draw [line] (test.west) -- ++(-2,0) |- node[anchor=south, xshift=0.5cm] {Nee (Foutanalyse)} (ontwerp.west);

	\end{tikzpicture}
	\caption{Iteratief ontwerpproces met terugkoppeling voor maakbaarheid (DFM).}\label{fig:design_cycle}
\end{figure}
