...
Lineair programmeren (LP) is een wiskundige methode om de beste uitkomst (zoals maximale winst of laagste kosten) te bepalen in een model waarvan de vereisten worden weergegeven door lineaire relaties.
...
\begin{figure}[ht]
    ...
    \begin{minipage}{0.45\textwidth}
        ...
        \begin{description}
            \item[\concept{Doelfunctie}] De formule die je wilt maximaliseren (winst) of minimaliseren (kosten). Bijvoorbeeld: $Z = 3x_1 + 5x_2$.
            \item[\concept{Beperkingen}] De limieten waarbinnen je moet werken (tijd, budget, grondstoffen). Deze vormen lijnen in de grafiek.
            \item[\concept{Toegelaten Gebied}] De zone die aan \textit{alle} beperkingen voldoet. De oplossing ligt altijd in dit gebied.
            \item[\concept{Optimale Oplossing}] Het punt in het toegelaten gebied waar de doelfunctie maximaal is. Dit is altijd een hoekpunt.
        \end{description}
    \end{minipage}%
    \hfill
    \begin{minipage}{0.5\textwidth}
...
Als je moet bepalen hoe je het meeste winst maakt afhankelijk van verschillende producten en beperkingen, dan gebruik je lineair programmeren. De grafische methode werkt goed voor 2 variabelen; voor meer variabelen gebruik je de Simplex-methode (via software).
...
\textbf{Cashflow} is het geld dat binnenkomt en uitgaat over een bepaalde periode.
...
\begin{description}
    \item[Horizont] De periode waarin je de investering bekijkt (bijv. 5 jaar).
    \begin{itemize}
...
\end{examenbox}
 ...
 \belangrijk{Actualisatie} is het proces van het omzetten van toekomstige cashflows naar hun huidige waarde.
    ...
    6.1446 is de annuïteitsfactor moet je opzoeken in een tabel. Je zoekt dan 10 jaar en 10\% op.
...
\begin{figure}[ht]
      \centering
        \includegraphics[width=0.45\textwidth]{balans1.png}
            \includegraphics[width=0.45\textwidth]{balans2.png}
